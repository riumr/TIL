...
for i in range(1,8):
  ...
  print(movie.director,movie.genre,movie.title,
        movie.opening_date,movie.running_time,movie.screening)
```
...
```python
for i in range(1,8):
  movie = Movie.objects.filter(director_id=i)
  for j in movie:
    director_name = Director.objects.get(id=j.director_id).name
    print(director_name,j.genre,j.title,
          j.opening_date,j.running_time,j.screening)
```
...
```python
for i in range(1,10):
  movie = Movie.objects.filter(genre=i)
  for j in movie:
    director_name = Director.objects.get(id=j.director_id).name
    genre_title = Genre.objects.get(id=j.genre_id).title
    print(director_name,genre_title,j.title,
        j.opening_date,j.running_time,j.screening)
```
...
```python
ordered_movie = Movie.objects.order_by('opening_date')
for i in ordered_movie:
   director_name = Director.objects.get(id=i.director_id).name
    genre_title = Genre.objects.get(id=i.genre_id).title
    print(director_name,genre_title,i.title,i.opening_date,
...
```python
ordered_movie = Movie.objects.order_by('opening_date')
first_opened = ordered_movie[0]
director_name = Director.objects.get(id=first_opened.director_id).name
genre_title = Genre.objects.get(id=first_opened.genre_id).title
print(director_name,genre_title,i.title,i.opening_date,
        i.running_time,i.screening)
```
...
```python
opening_movie = Movie.objects.filter(screening=True)
for i in opening_movie:
  director_name = Director.objects.get(id=i.director_id).name
  genre_title = Genre.objects.get(id=i.genre_id).title
  print(director_name,genre_title,i.title,i.opening_date,
        i.running_time,i.screening)
...
```python
name_id = Director.objects.get(name='봉준호').id
b_movie = Movie.objects.filter(id=name_id)
```
